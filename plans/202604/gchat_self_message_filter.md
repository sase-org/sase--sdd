...
description: Stop retired chat plugin from launching agents in response to its own
  outbound messages. The current ``sender.type == "BOT"`` filter never matches because
  the gchat CLI authenticates as the user, so sase's own posts appear as inbound user
...
status: done
prompt: sdd/plans/202604/prompts/gchat_self_message_filter.md
tier: tale
---
